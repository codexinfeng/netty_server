...
public class ModuleMetricsProcessor {
	...
	private String buildHistogram(CompositeData data) {
		CompositeDataSupport histogram = (CompositeDataSupport) data
				.get("histogram");
		...
		for (; i < ranges.length; i++) {
			distribute.append(TR_BEGIN + TD_BEGIN
					+ ((i == 0) ? i : ranges[i - 1]) + "~" + ranges[i]
					+ "毫秒的笔数：" + invokeHistogram[i] + BR + TD_END + TR_END);
		}
		distribute.append(TR_BEGIN + TD_BEGIN + "大于等于" + ranges[i - 1]
				+ "毫秒的笔数：" + invokeHistogram[i] + BR + TD_END + TR_END);
		...
		try {
			...
			if (obj instanceof CompositeData[]) {
				for (CompositeData compositeData : (CompositeData[]) obj) {
					...
					long invokeFilterCount = (Long) (data
							.get("invokeFilterCount"));
					long invokeTimespan = (Long) (data.get("invokeTimespan"));
					long invokeMinTimespan = ((Long) (data
							.get("invokeMinTimespan")))
							.equals(Long
									.valueOf(ModuleMetricsVisitor.DEFAULT_INVOKE_MIN_TIMESPAN)) ? Long
							.valueOf(0L) : (Long) (data
							.get("invokeMinTimespan"));
					long invokeMaxTimespan = (Long) (data
							.get("invokeMaxTimespan"));
					String lastStackTraceDetail = (String) (data
							.get("lastStackTraceDetail"));
					...
					metrics.append(TD_BEGIN
							+ (lastErrorTime != null ? lastErrorTime : "")
							+ TD_END);
