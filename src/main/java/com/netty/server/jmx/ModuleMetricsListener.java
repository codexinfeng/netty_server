...
import com.netty.server.event.AbstractInvokeEventBus;
...
public class ModuleMetricsListener implements NotificationListener {
	...
	public void handleNotification(Notification notification, Object handback) {
		...
		AbstractInvokeEventBus.ModuleEvent event = Enum.valueOf(
				AbstractInvokeEventBus.ModuleEvent.class,
				acn.getAttributeType());
		ModuleMetricsVisitor visitor = ModuleMetricsHandler.getInstance()
				.visit(acn.getMessage(), acn.getAttributeName());
		...
		case INVOKE_TIMESPAN_EVENT:
			...
			visitor.getHistogram().record(
					((Long) acn.getNewValue()).longValue());
			break;
