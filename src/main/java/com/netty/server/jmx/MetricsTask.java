...
public class MetricsTask implements Runnable {
	...
	public MetricsTask(CyclicBarrier barrier,
			List<ModuleMetricsVisitor> visitorList) {
	...
	private void count(List<ModuleMetricsVisitor> list) {
		for (int i = 0; i < result.size(); i++) {
			...
			if (metrics.getLastErrorTime() > 0) {
				result.get(i).setErrorLastTimeLongVal(
						metrics.getLastErrorTime());
				result.get(i).setLastStackTraceDetail(
						metrics.getLastStackTraceDetail());
			}

		}
	}

	private void merge(int index, List<ModuleMetricsVisitor> list,
		...
		for (int i = 0; i < list.size(); i++) {
			boolean find = equals(result.get(index).getModuleName(), list
					.get(i).getModuleName(), result.get(index).getMethodName(),
					list.get(i).getMethodName());
			if (find) {
				...
				if (fail > 0) {
					...
					if (lastTime > lastErrorTime) {
						...
						lastStackTraceDetail = list.get(i)
	...
	private void accmulate() {
		...
		while (iterator.hasNext()) {
			ModuleMetricsVisitor visitor = (ModuleMetricsVisitor) iterator
					.next();
			result.add(new ModuleMetricsVisitor(visitor.getModuleName(),
	...
	private boolean equals(String srcModuleName, String destModuleName,
			String srcMethodName, String destMethodName) {
		return srcModuleName.equals(destModuleName)
				&& srcMethodName.equals(destMethodName);
	}
